...
filename = 'aalh_iit_buildings_010.xlsx'
...
for row in ws.iter_rows(min_row=minimumrow, min_col=minimumcol, max_row=maximumrow, max_col=maximumcol):
    for cell in row:
        ameryear = None
        yearraw = str(ws.cell(row=iterationrow, column=targetcol).value)
        if yearraw.find(',') != -1:
    ...
    for cell in row:
        amermon = None
        monraw = str(ws.cell(row=iterationrow, column=targetcol).value)
        if monraw.find(',') != -1:
    ...
    for cell in row:
        amerday = None
        dayraw = str(ws.cell(row=iterationrow, column=targetcol).value)
        if dayraw.find(',') != -1:
    ...
    for cell in row:
        ...
        if testvar.find('/') != -1:
            ...
            ws.cell(row=iterationrow, column=isostandardcol).value = isodate
            print('ROW:',iterationrow,'|',testvar,'|',isodate)
            #print(isodate)
        #else:
            #continue
    ...
    for cell in row:
        if ameryear == None:
            ...
            print('ROW:',iterationrow,'|',yearraw,'|',ws.cell(row=iterationrow, column=isostandardcol).value)
    iterationrow = iterationrow + 1
wb.save('aalh_iit_buildings_010.xlsx')