...
for row in ws.iter_rows(min_row=minimumrow, min_col=minimumcol, max_row=maximumrow, max_col=maximumcol):
    for cell in row:
        print(iterationrow)
        removetoledo = ws.cell(row=iterationrow, column=covcol).value
        if removetoledo == None:
            print('Nothing here, move along')
        elif removetoledo == 'Toledo, Ohio':
            removetoledofinal = removetoledo.replace("Toledo, Ohio", "")
            ws.cell(row=iterationrow, column=covcol).value = removetoledofinal
        elif removetoledo.find('Toledo, Ohio') != -1:
            removetoledolist = removetoledo.split(',')
            removetoledoaddress = removetoledolist[0]
            removetoledofinal = removetoledoaddress.strip()
            ws.cell(row=iterationrow, column=covcol).value = removetoledofinal
    ...
    for cell in row:
        ...
        if testvar == None:
        ...
        elif testvar.find('Lucas County') != -1:
            ws.cell(row=iterationrow, column=targetcol).value = 'Lucas County (Ohio)'
            print(ws.cell(row=iterationrow, column=targetcol).value)
            ...
            print(ws.cell(row=iterationrow, column=targetcol).value)
        elif testvar.find('Palmyra') != -1:
            ws.cell(row=iterationrow, column=targetcol).value = 'Palmyra Township (Michigan); Lenawee County (Michigan)'
            print(ws.cell(row=iterationrow, column=targetcol).value)
